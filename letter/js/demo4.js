    ...
    enter(direction = "down") {
      ...
      this.DOM.title.letters.forEach((letter, pos) => {
        ...
        if (anime.random(0, 1) > 0.5) {
          letterAnim.translateX = [
            anime.random(0, 1) > 0.5 ? -window.innerWidth : window.innerWidth,
            0,
          ];
        } else {
          letterAnim.translateY = [
            anime.random(0, 1) > 0.5 ? -window.innerHeight : window.innerHeight,
            0,
          ];
        }
        anime(letterAnim);
